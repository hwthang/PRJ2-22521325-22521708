...
// ================= Dropdown Đoàn Trực Thuộc =================
function ParentUnionDropdown({ selected = [], onChange }) {
  const unions = [
    { value: "cntt", label: "Đoàn Khoa Công nghệ thông tin" },
    { value: "tt", label: "Đoàn Khoa Truyền thông" },
    { value: "kt", label: "Đoàn Khoa Kinh tế" },
    { value: "luat", label: "Đoàn Khoa Luật" },
  ];
  ...
  const renderSummary = () => {
    if (selected.length === 0) return "Chọn đoàn trực thuộc";
    if (selected.length <= 2)
      return unions
        .filter((u) => selected.includes(u.value))
        .map((u) => u.label)
        .join(", ");
    return `${selected.length} đoàn đã chọn`;
  };
...
// ================= Header Component =================
function ChapterHeader({ onRefresh, onAdd }) {
  return (
    <div className="flex flex-col md:flex-row gap-6 justify-between items-center">
      <p className="font-medium text-2xl text-nowrap">Danh sách chi đoàn</p>
        ...
        <div className="bg-white rounded-lg p-6 flex flex-col gap-6 relative">
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-6 w-full">
            <div className="flex-1 flex items-end">
              <ParentUnionDropdown selected={unions} onChange={setUnions} />
            </div>
            <div className="flex-1 flex items-end">
