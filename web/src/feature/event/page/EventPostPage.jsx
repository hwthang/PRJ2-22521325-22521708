...
const EventPostPage = () => {
  ...
  const [interaction, setInteraction] = useState({ isLiked: false, likeCount: 0 });
...
const handleLike = async () => {
  ...
  useEffect(() => {
    if (events.length > 0 && eventIdFromQuery) {
      ...
      if (found) {
        ...
        setInteraction({ isLiked: !!found.isLiked, likeCount: found.postId?.likes ?? 0 });
      }
    }
  ...
  const handleRegister = async (eventId) => {
    try {
      ...
      setEvents(prev => prev.map(ev => ev._id === eventId ? { ...ev, hadRegistered: true } : ev));
    } catch (error) {
...
/>
          ...
          <div className="flex flex-col items-center justify-center text-slate-300 opacity-50 h-full">
            ...
            <p className="font-bold uppercase text-xs">Không tìm thấy sự kiện phù hợp</p>
          </div>
        )}
      </main>
    </div>
