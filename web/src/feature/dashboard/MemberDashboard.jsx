...
import { 
  Edit2, User, Trophy, Mail, Phone, MapPin, 
  Calendar, Award, BookOpen, Globe, Hash, 
  X, Camera, Star, ShieldCheck, GraduationCap, 
  Briefcase, Zap, Crown, Check, ChevronRight,
  ClipboardList, ShieldAlert, FileText, CalendarDays, Loader2
} from "lucide-react";
...
const MemberDashboard = () => {
  ...
  // --- LOGIC FETCH THÔNG TIN THÀNH VIÊN ---
      ...
      <div className="max-w-6xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-blue-100/50 border border-blue-50 flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            <div className="relative group">
              <div className="w-24 h-24 rounded-3xl overflow-hidden ring-4 ring-blue-50 shadow-2xl">
                <img 
              ...
              {isEditing && (
                <button 
                  ...
                  className="absolute -bottom-2 -right-2 bg-blue-600 text-white p-2 rounded-xl shadow-lg hover:bg-blue-700 transition-all hover:scale-110"
                >
                  <Camera size={16} />
                </button>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-blue-50">
              <div className="flex items-center gap-3 mb-8 border-b border-blue-50 pb-6">
                <div className="p-2.5 bg-blue-600 rounded-2xl text-white shadow-blue-200 shadow-lg"><User size={20}/></div>
            ...
            {/* Section 2: Khen thưởng & Kỷ luật (NEW) */}
            <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-blue-50">
              <div className="flex items-center justify-between mb-8 border-b border-blue-50 pb-6">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-emerald-600 rounded-2xl text-white shadow-emerald-200 shadow-lg"><ClipboardList size={20}/></div>
                  <h3 className="text-[13px] font-black text-blue-900 uppercase tracking-[0.2em]">Khen thưởng & Kỷ luật</h3>
                </div>
                <div className="text-[10px] font-black text-slate-400 bg-slate-50 px-3 py-1 rounded-lg">Lịch sử đánh giá</div>
              </div>
              ...
              <div className="space-y-4">
                {evalLoading ? (
                  ...
                  evaluations.map((item) => (
                    <EvaluationCard key={item._id} data={item} />
                  ))
                ) : (
            ...
            {/* Section 3: Hồ sơ đoàn viên */}
            <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-blue-50">
              <div className="flex items-center gap-3 mb-8 border-b border-blue-50 pb-6">
                <div className="p-2.5 bg-indigo-600 rounded-2xl text-white shadow-indigo-200 shadow-lg"><ShieldCheck size={20}/></div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <section className="bg-white rounded-[2.5rem] p-7 shadow-2xl shadow-blue-200/40 border border-blue-50 overflow-hidden relative sticky top-6">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="relative">
                            <img 
                              ...
                              className={`w-11 h-11 rounded-2xl object-cover ring-2 ${index === 0 ? 'ring-yellow-400/50' : 'ring-white shadow-sm'}`} 
                              alt="" 
                            />
                            <div className={`absolute -top-2 -left-2 w-6 h-6 rounded-xl flex items-center justify-center text-[10px] font-black shadow-lg
                              ${index === 0 ? 'bg-yellow-400 text-white' : index === 1 ? 'bg-slate-300 text-white' : index === 2 ? 'bg-orange-400 text-white' : 'bg-white text-blue-400 border border-blue-50'}`}>
                              {index + 1}
                            </div>
                          </div>
                          <div className="min-w-0">
                            <p className={`text-[12px] font-black truncate uppercase tracking-tight ${member.memberId === memberId ? 'text-white' : 'text-blue-900'}`}>
                              {member.fullName}
                            </p>
                            <div className={`flex items-center gap-1.5 text-[10px] font-bold ${member.memberId === memberId ? 'text-blue-200' : 'text-blue-400'}`}>
                              <Zap size={10} fill="currentColor" className="animate-pulse"/> {member.score.toLocaleString()} PTS
                            </div>
                          </div>
                        </div>
                        {index < 3 && <Award size={index === 0 ? 22 : 18} className={index === 0 ? 'text-yellow-400' : index === 1 ? 'text-slate-300' : 'text-orange-400'} />}
                      </div>
                    ))
                  ) : (
                    <div className="py-10 text-center text-slate-400 font-bold italic">Chưa có dữ liệu xếp hạng</div>
                  )}
                </div>

                {/* MY RANK CARD */}
                <div className="mt-8 pt-7 border-t border-dashed border-blue-100">
                  <div className="bg-[#0F172A] rounded-[2rem] p-6 text-white shadow-2xl shadow-blue-900/40 relative overflow-hidden group">
                    ...
                    <div className="relative flex justify-between items-center mb-6">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-5xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white to-slate-500">
                              #{myRankInfo.rank || '--'}
                            </span>
                            <span className="text-[11px] font-black text-slate-500 uppercase">/ {leaderboard.length}</span>
                        </div>
                      </div>
                      <div className="bg-blue-600/20 px-4 py-2.5 rounded-2xl border border-blue-500/30 backdrop-blur-xl">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="w-full bg-slate-800/50 h-2.5 rounded-full overflow-hidden p-0.5 border border-slate-700/50">
                        <div 
                          className="bg-gradient-to-r from-blue-600 via-indigo-500 to-cyan-400 h-full rounded-full shadow-[0_0_15px_rgba(37,99,235,0.5)] relative transition-all duration-1000 ease-out" 
...
const InputField = ({ label, value, isEditing, onChange, type = "text" }) => (
  <div className="space-y-2.5 group">
    ...
    {isEditing ? (
      ...
      <div className="bg-blue-50/20 rounded-2xl px-5 py-4 text-sm font-black text-slate-800 border border-transparent group-hover:bg-blue-50/50 group-hover:border-blue-100 transition-all cursor-default">
...
const SideInfo = ({ label, value, icon }) => (
  <div className="flex items-center justify-between p-5 bg-blue-50/30 rounded-[1.8rem] border border-blue-100/30 hover:bg-white hover:shadow-xl hover:border-blue-200 transition-all group cursor-default">
    <div className="flex items-center gap-4 text-blue-400 group-hover:text-blue-600 transition-colors">
      <div className="p-2 bg-white rounded-xl shadow-sm border border-blue-50 group-hover:scale-110 transition-transform">{icon}</div>
...
// 3. EvaluationCard (NEW)
const EvaluationCard = ({ data }) => {
  const isReward = data.type === "reward";
  return (
    <div className={`group relative bg-white rounded-[2rem] border-l-4 p-5 shadow-sm hover:shadow-md transition-all duration-300 border-slate-100 ${
      isReward ? "border-l-emerald-500 bg-emerald-50/10" : "border-l-rose-500 bg-rose-50/10"
    }`}>
