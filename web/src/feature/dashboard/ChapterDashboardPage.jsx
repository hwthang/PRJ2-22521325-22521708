...
const ChapterDashboardPage = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [
        ...
        fetch(`${API_URL}/members?chapterId=${chapterId}`).then((r) =>
          r.json()
        ),
        fetch(`${API_URL}/events?chapterId=${chapterId}`).then((r) => r.json()),
        fetch(`${API_URL}/documents?chapterId=${chapterId}`).then((r) =>
          r.json()
        ),
        fetch(`${API_URL}/surveys?chapterId=${chapterId}`).then((r) =>
          r.json()
        ),
        fetch(`${API_URL}/members/leaderboard?chapterId=${chapterId}`).then(
          (r) => r.json()
        ),
      ...
      setStats({
        members: allMembers,
        eventsCount:
          resEvents.data?.total || resEvents.data?.events?.length || 0,
        docsCount: resDocs.data?.total || resDocs.data?.documents?.length || 0,
        surveysCount:
          resSurveys.data?.total || resSurveys.data?.surveys?.length || 0,
        board: boardMembers,
      });
  ...
  const handleCopyId = (e) => {
    e.preventDefault(); // Ngăn chặn Link nhảy trang nếu ID nằm trong thẻ Link
    e.stopPropagation(); // Ngăn chặn sự kiện nổi bọt

    if (chapterId) {
      navigator.clipboard.writeText(chapterId.toUpperCase());
      toast.success("Đã sao chép mã ID!"); // Nếu bạn dùng react-toastify
    }
  };
  if (loading) {
    return (
            ...
            <div className="text-center md:text-left">
              ...
              <div className="flex flex-wrap justify-center md:justify-start items-center gap-5 mt-3 text-slate-400">
                ...
                <span
                  ...
                  className="group flex items-center gap-1.5 cursor-pointer active:scale-95 transition-all"
                  title="Click để sao chép"
                >
                  <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-2 py-0.5 rounded border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                    ID: {chapterId}
                  </span>
                  <Copy
                    size={12}
          ...
          <div className="lg:col-span-4 space-y-8">
              ...
              <div className="space-y-3 max-h-[460px] overflow-y-auto pr-2 custom-scrollbar">
                {leaderboard.length > 0 ? (
                  leaderboard.map((m, i) => (
                    <button
                      key={m.memberId}
                      onClick={() => navigate(`/app/chapter/members/${m.memberId}`)}
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          ...
                          />
                          <div
                            className={`absolute -top-2 -left-2 w-6 h-6 rounded-lg text-[10px] font-black text-white flex items-center justify-center shadow-lg ${
                              i === 0
                                ? "bg-yellow-500"
                                : i === 1
                                ? "bg-slate-300"
            ...
            <section className="bg-slate-900 p-7 rounded-[2.5rem] shadow-2xl text-white">
              ...
              <div className="grid grid-cols-2 gap-4">
                {stats.board.map((b, i) => (
                  <div
                   onClick={() => navigate(`/app/chapter/members/${b._id}`)}
                    key={i}
                    className="p-4 bg-white/5 rounded-3xl border border-white/5 text-center transition hover:bg-white/10"
                  >
                    <img
                      ...
                      alt=""
                    />
                    <p className="font-black text-[10px] uppercase leading-tight line-clamp-1">
                      {b.fullName}
                    </p>
                    <p className="text-[9px] font-bold text-blue-400 uppercase mt-1 tracking-tighter">
                      {b.position}
                    </p>
                  </div>
                ))}
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-pink-50/50 p-4 rounded-3xl">
                      <p className="text-[10px] font-black uppercase text-pink-400 mb-1">
                        Nữ giới
                      </p>
...
const StatCard = ({ label, value, icon, color }) => (
  <div className="group bg-white p-7 rounded-[2.5rem] border border-white shadow-xl shadow-slate-200/60 hover:-translate-y-1 transition-all duration-300">
    <div
      className={`${color} w-14 h-14 rounded-2xl flex items-center justify-center text-white mb-6 shadow-lg shadow-blue-100 group-hover:scale-110 transition-transform`}
    >
      {icon}
    </div>
