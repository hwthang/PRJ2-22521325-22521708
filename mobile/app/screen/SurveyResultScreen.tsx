...
import { User, Calendar, MapPin, CheckCircle2, ClipboardList } from "lucide-react-native";
...
const SurveyResultScreen = () => {
  ...
  if (loading) return <View style={styles.center}><Text>Đang tải kết quả...</Text></View>;
  if (!result) return <View style={styles.center}><Text>Không tìm thấy dữ liệu</Text></View>;
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerCard}>
          <View style={styles.profileRow}>
            <Image
              source={{ uri: member.accountId?.avatar?.url }}
              style={styles.avatar}
            />
          ...
          <View style={styles.infoGrid}>
            <View style={styles.infoItem}>
              ...
              <Text style={styles.infoValue}>{formatDateToDDMMYYYY(completedAt)}</Text>
            </View>
            <View style={styles.infoItem}>
              <MapPin size={16} color="#64748b" />
              <Text style={styles.infoValue} numberOfLines={1}>{member.address}</Text>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  typeTag: { backgroundColor: "#f1f5f9", paddingHorizontal: 6, borderRadius: 4 },
  typeTagText: { fontSize: 10, color: "#94a3b8", fontWeight: "700" },
  questionText: { fontSize: 15, fontWeight: "600", color: "#1e293b", marginBottom: 12 },
  answerBox: {
    flexDirection: "row",
